...
Historical `dry-run:not-sent` rows are displayed as unsent immediately. Retrying clears their fake provider ID and timestamp and recomposes from current evidence if the draft has no matching source fingerprint. No bulk data mutation is required.
...
Each new draft stores a SHA-256 source fingerprint covering its extraction, verdict, organization information, evidence, family signature and recipient context. Saving checks the composer's fingerprint against current data; claiming a send and the threading fallback check again. Regenerated evidence IDs and unchanged registry refresh timestamps do not invalidate it. A changed source clears or recomposes only a provably unattempted draft. Attempted, accepted or unknown outcomes retain their exact text and show an outdated-advice warning; changed or unstamped attempted drafts cannot retry automatically. A late provider acceptance is still recorded truthfully. The guard cannot recall a request already sent to the provider.
